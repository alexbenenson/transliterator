...
/**
  ...
  nodeDecorator - function(node), returns replaced styles - call to decorate the node with borders
  nodeUndecorator - function(node, savedStyle) - call to restore normal style, pass the return from decorator
*/
var MappingState = exports.MappingState = function MappingState(node, commandKey, keyHandler, removeHandler, stateID, nodeDecorator, nodeUndecorator) {
  this.node = node;

  console.debug(node.localName);
  ...
  */
  ...
  this.undecorator = nodeUndecorator;
  ...
  if (!this.lightweight) {
    ...
    */
    if (nodeDecorator)
        this.savedStyle = nodeDecorator(node);
  }
  ...
  clear: function() {
    ...
    if (!this.lightweight) {
      ...
      */
      ...
      if (this.undecorator && this.savedStyle)
        this.undecorator(this.savedStyle);
    }
