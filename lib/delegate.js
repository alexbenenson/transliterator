...
TransliteratorWindowDelegate.prototype = {
  ...
  windowKeyMappingListener: function(event) {
    ...
    // ignore if no charCode (e.g. arrows) or if modifiers are pressed
    // ignore events with keyCode=255 or keyCode=8 and charCode=0 (events generated here)
    // TODO: filter by phase as well?
    if (event.charCode > 0 && !event.ctrlKey && !event.altKey && !event.metaKey && !( (event.keyCode == 255) || (event.keyCode == 8 && event.charCode == 0))) {
