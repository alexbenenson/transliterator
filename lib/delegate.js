...
var TransliteratorWindowDelegate = exports.TransliteratorWindowDelegate = function TransliteratorWindowDelegate(service, wnd) {
  ...
  // all-window mapping state vars
  this.isKeyMappingMode = false;
  this.savedStyle    = {};
  this.keyMappingModeCommandKey = "";
...
TransliteratorWindowDelegate.prototype = {
  ...
  init: function() {
    ...
    this.boundWindowKeyMappingListener = this.windowKeyMappingListener.bind(this);
  },
  ...
  addShortcuts: function() {
    ...
    if (this._service.getOptions()[Constants.OVERRIDE_SHORTCUTS])
      try {
      	...
      	// this behavior is controlled by pref overrideShortcuts
    ...
    for (var i = 0; i < markers.length; i++) {
      ...
      var popupHandler = function(event) {
        ...
        for (var cmd in theItems) {
          ...
          else if (endPoint.actionType == EndPoint.MAP_ALL) {
            menuItem.setAttribute("hidden", false);
            menuItem.setAttribute("checked", this_.isKeyMappingMode);
            visibleMenuItems ++;
          }
        }
        if (separator)
  ...
  processCommand: function(command) {
    ...
    if (command == "transliterator_options") {
      ...
      if (endPoint.actionType == EndPoint.BATCH)
        ...
        this.toggleWindowKeyMapper(endPoint);
  ...
  showOptionsDialog: function() {
    ...
    this.getWindow().openDialog("chrome://transliterator/content/prefs/prefs.xul", "dlg", "chrome,dialog,centerscreen,resizeable=on");
  },
  ...
  toggleKeyMapper: function(endPoint) {
    ...
    // if in all-window mapping mode, switch it off
    this.endKeyMappingMode();
    ...
    if (state && state.key == endPoint.commandKey) {
      ...
      state.reset(); //TODO may need more than reset here - maybe close old state and init new state
    }
    else {
      // toggle on
      var newID = this.getService().getUniqueID();
      state = new MappingState(node, endPoint.commandKey, 
          this.keypressMappingHandler.bind(this), this.removeMappingState.bind(this), 
          newID, 
          this.decorateNode.bind(this), this.removeNodeDecorations.bind(this));
  ...
  toggleWindowKeyMapper: function(endPoint) {

    if (this.isKeyMappingMode) {
      this.endKeyMappingMode(endPoint);
    }
    else {
      // turn on window key mapper
      this.startKeyMappingMode(endPoint);
    }

  },

  startKeyMappingMode: function(endPoint) {
    if (this.isKeyMappingMode)
      return;

    // turn on window key mapper

    var docEl = this.getWindow().document.documentElement;

    //remove all mapping states (in case there are any field-level states)
    this.removeMappingStates();    


    // add event listener 
    this.getWindow().document.addEventListener("keypress", this.boundWindowKeyMappingListener, true);

    // save decorations
    this.savedStyle.border = docEl.style.border;
    this.savedStyle.outline = docEl.style.outline;
    this.savedStyle.margin = docEl.style.margin;
    // set new decorations
    //docEl.style.border = "dotted 1px #3399FF";

    docEl.style.outline = this.getStyle();

    docEl.style.margin  = this.getService().getOptions()[Constants.BORDER_WIDTH];

    this.isKeyMappingMode = true;
    this.keyMappingModeCommandKey = endPoint.commandKey;

  },

  getStyle: function() {
    var opts = this.getService().getOptions();
    

    return opts[Constants.BORDER_STYLE] + " " + 
           opts[Constants.BORDER_WIDTH] + " " + 
           opts[Constants.BORDER_COLOR];

  },

  endKeyMappingMode: function(endPoint) {
    if (!this.isKeyMappingMode)
      return;

    // turn off window key mapper
    var docEl = this.getWindow().document.documentElement;

    // remove event listener window.document.removeEventListener("keypress", handler, true);
    this.getWindow().document.removeEventListener("keypress", this.boundWindowKeyMappingListener, true);
    
    //remove all existing mapping states (window-level)
    this.removeMappingStates();

    // restore decorations
    for (var p in this.savedStyle) {
      docEl.style[p] = this.savedStyle[p];
    }

    //docEl.style.border = this.savedStyle.border || "";
    //docEl.style.outline = this.savedStyle.outline || "";
  ...
  // find the editor containing given window
  findEditorForWindow: function(wnd) {
      var doc = this.getWindow().document;

      // try via activeElement (blue griffon, others?)
      if (doc.activeElement && 
          doc.activeElement.tagName == "editor" && 
          doc.activeElement.contentWindow == wnd)
          return doc.activeElement;


      // find all editors, see which one contains the given window
      // is this even needed anymore? (worked in tbird but not bluegriffon)
      var editors = doc.getElementsByTagName("editor");
      for (var i = 0; i < editors.length; i++) {
          if (editors[i].contentWindow == wnd)
              return editors[i];
    ...
    if (!node) {
      if (document.commandDispatcher.focusedWindow) {
        ...
        if (document.commandDispatcher.focusedWindow.document ) {
          ...
          var editor = this.findEditorForWindow(document.commandDispatcher.focusedWindow);
          if (editor)
              return editor.contentDocument.documentElement;
  ...
  // get up to @param count characters from node before offset
  getBackBuffer : function (node, offset, count) {
    ...
    // instead of checking if this is an editor, check if it has "value" property, and if not then assume that it's html in rich text edit mode
    // TODO - check this for regressions!!!
    else {
      var selection = this.getSelection(node);

      if (!selection.focisNode || !selection.focusNode.nodeValue)
        return "";
  ...
  decorateNode: function(node) {
    // choose the correct node to decorate (node or node.ownerDocument.defaultView.frameElement)
    // apply style (outline or border)
    // return an object containing decorated node and saved style
    ...
    if (node.ownerDocument.documentElement == node && 
       ...
       var node = node.ownerDocument.defaultView.frameElement;
       ss["border"] = node.style.border;
       node.style.border = style; 
    }
    else if (node.ownerDocument.documentElement == node && 
      (editor = this.findEditorForWindow(node.ownerDocument.defaultView))
      ) {
      // xul editor
      node = editor;
      ss["border"] = node.style.border;
  ...
  //@param ss - object containing decorated node and saved style (returned from decorateNode())
  removeNodeDecorations: function(ss) {
    

    for (var p in ss.style) {
      ss.node.style[p] = ss.style[p];
    }
  },
  ...
  keypressMappingHandler: function(event) {
    ...
    if ((((event.key == "") || (event.keyCode == 255)) && event.charCode > 0) || event.keyCode == 8) {
    ...
    if (event.charCode > 0 && !event.ctrlKey && !event.altKey && !event.metaKey) {
      ...
      for (var i = 0; i < tosend.length; i++) {
        ...
        if (tosend[i] == 8)
          ...
          evt.initKeyEvent(event.type, true, event.cancelable, null, false, false, false, false, (evt.key != undefined) ? 0 : 255, tosend[i]);
  ...
  windowKeyMappingListener: function(event) {
    // process  document-level key events in windowKeyMapping mode


    // ignore if no charCode (e.g. arrows) or if modifiers are pressed
    // ignore events with keyCode=255 or keyCode=8 and charCode=0 (events generated here) or key=="" and charCode > 0
    // TODO: filter by phase as well?
    if (event.charCode > 0 && !event.ctrlKey && !event.altKey && !event.metaKey && 
    		!( 
    				(event.key == "") || (event.keyCode == 255)
    		 )
    	) {

      // determine event target eligibility
      // NOTE: this.activeNode() does not always return the same node as event.originalTarget. OriginaLTarget is more fine-grained, e.g. returns 
      // HtmlBodyElement instead of HtmlHtmlElement in Midas editor (https://developer.mozilla.org/en-US/docs/Midas, http://www-archive.mozilla.org/editor/midasdemo/)
      // for now - will go with whatever the event contains, regardless of editability. maybe need to develop a method for checking editability similar to getActiveNode()
      // 

      var node = event.originalTarget;

      //locate or create mapping state for the node
      var state = this.getMappingState(node);
      if (!state) {
          var id = this.getService().getUniqueID();
          state = new MappingState(node, this.keyMappingModeCommandKey, null, this.removeMappingState.bind(this), id);
          this.addMappingState(state, id);
      }



      //(the following code is copied from the other handler with some mods - TODO - extract into a  function)
      ...
      // array of characters to send to the node - backspaces for each char that needs to be replaced, then all replaced and new chars
      var tosend = new Array(result.replace + result.out.length);
      for (var i = 0; i < result.replace; i++)
        tosend[i] = 8; // send backspace for all those that need to be removed
      for(var i = result.replace; i < tosend.length; i++)
      ...
      var dispatched = !(tosend.length > 0);

      // send one event per each item in tosend
      for (var i = 0; i < tosend.length; i++) {

        var evt = node.ownerDocument.createEvent("KeyEvents");

        if (tosend[i] == 8)
          evt.initKeyEvent(event.type, true, event.cancelable, null, false, false, false, false, 8, 0);
        else
            // use keyCode 255 as an indicator that the event should be ignored
        	// using on event key (will be empty) if it is available, keyCode=255 otherwise
        	// keyCode=255 did not play nice with Google Docs as of 12/06/2014
          evt.initKeyEvent(event.type, true, event.cancelable, null, false, false, false, false, 
        	(evt.key != undefined) ? 0 : 255, 
        		  tosend[i]);
    ...
    return true;
  }  // end of windowKeyMappingListener
